<%@page contentType="text/html" pageEncoding="UTF-8" %><%@ page import="java.net.*,javax.xml.parsers.*,org.w3c.dom.*,org.xml.sax.*,java.util.*,java.util.Arrays,java.io.*,java.util.regex.*,java.net.URLEncoder,hk.edu.csids.bookquery.*,hk.edu.csids.*,org.apache.commons.lang3.*"%>
<%
	...
	StringHandling stringHandle = new StringHandling ();
	locate_base = stringHandle.trimSpecialChars(locate_base);
	locate_base = stringHandle.normalizeString(locate_base);
	String urlStr = "http://" + server_adr + "/X?op=ill-get-doc&doc_number=" + doc_number
		 + "&library=" + doc_library;
	String urlStr2 = "http://" + server_adr + "/X?op=ill-get-doc-short&doc_number=" + doc_number
		 + "&library=" + doc_library;
	String outstr = "";
	...
	Query q;
	if(z3950Search){
		q = new Z3950QueryByISBN(req_isbn, inst);
	} else {
	 	q = new PrimoQueryByISBN(req_isbn, inst);
	 } //end if
	...
	ans_loa_no_isbn = ans_ava_no_isbn; 
		...
		if(z3950Search){
	q2 = new Z3950QueryByNonISBN(req_author, req_title, req_publisher, req_publishingyear, req_edition, req_vol, inst);
		} else {
	q2 = new PrimoQueryByNonISBN(req_author, req_title, req_publisher, req_publishingyear, req_edition, req_vol, inst);
		} //end if
		if(q2.match()){ 
	...
	+ ans_loa_no_isbn
	+ "</loanable-items>"
	+ "<set-number>000002</set-number>"
	+ "<bib-library></bib-library>"
	+ "<bib-doc-number>000000000</bib-doc-number>"
	+ "<locate-error-code>00</locate-error-code>"
	+ "</line>";
	} //end if
