...
<br>
<li>
File "ccconfig.txt" ("COPYCAT_QUERY_LIST"): "COPYCAT_QUERY_LIST" is the whole list of institute codes of libraries to make Z39.50 query to. The Z39.50 server details associated with the institute code must be set before, in bqconfig.txt, for this setup to work.
</li>
<br>
<li>
File "ccconfig.txt" ("EXISTTAG_FOR_MATCHING") defines the MARC tags that only if the return records contain such tags, CopyCAT program will include the records on reports; the first 3 chars is a MARC tag value, the fourth is a subfield or control field indicator "_" which follows a control number; "ALL" will disable this function. e.g. "300a" means records will be retrived only if the records have MARC tag 300 subfield a; "6XX_0" means records will be retrived only if records have MARC tag starting with "6" and have 2nd indicator field "0"
</li>
<br>
<li>
File "ccconfig.txt"("TOT_DIGIT_RECORDID"): defines total number of the ILS record ID, and add "0" to the left if ILS record IDs don't have enough digits (to be used with the function "con001"). 
</li>
<br>
<li>
File "ccconfig.txt" ("CORRUPT_RECORD_STRING") defines strings which determine if a returned record, which contains the string(s), is corrupted. 
</li>
<br>
<li>
