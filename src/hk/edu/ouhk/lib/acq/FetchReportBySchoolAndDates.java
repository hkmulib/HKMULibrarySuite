...
public class FetchReportBySchoolAndDates {
	private String sql;
	 ...
	 */
	public FetchReportBySchoolAndDates(String schCode, String writePath, Writer wr) {
		init(wr);
		setSchCode(schCode);
	 ...
	 */
	public FetchReportBySchoolAndDates(String schCode, String startDate, String endDate, String writePath, Writer wr) {
		init(wr);
	...
	public void fetchReport() {
		...
		try {
			...
			if (monthReport) {
				String filename = writePath + "/" + schCode + "-Monthly Report-" + lastMonth + " " + lastYear + ".xlsx";
			...
			outputHTML += "<table id='reportTable' class='table table-hover sortable'> <thead> <tr> <td> No. </td> <td width=5%> LIB ref# </td> <td> Title </td> <td> Author / Editor </td> <td> Link to the item </td> <td> Publisher </td> <<td> Call # </td> <td> Order Material Type </td> <td> SID Date </td> <td> Budget # </td> </tr> </thead>\n";
			outputHTML += "<tbody>";
			...
			for (String str : al) {
				...
				String reportLine = "<tr> <td>#</td><td>" + result.get(str)[0] + "</td><td>" + result.get(str)[1]
						...
						+ "' target=_blank> click here </a> </td> <td>" + result.get(str)[3] + "</td><td>"
						+ result.get(str)[4] + "</td> <td>" + result.get(str)[5] + "</td> <td>" + result.get(str)[6]
						+ "</td><td>" + result.get(str)[7] + "</td></tr>\n";
			...
			reportLog = "Report generation success. ";
